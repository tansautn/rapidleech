...
class keep2share_cc extends DownloadClass {
	...
	private function Login($user, $pass) {
		...
		is_notpresent($page, '/auth/logout.html">', 'Login Error.');
	...
	private function CookieLogin($user, $pass, $filename = 'keep2share_dl.php') {
		...
		if (array_key_exists($hash, $savedcookies)) {
			...
			if (stripos($page, '/auth/logout.html">') === false) return $this->Login($user, $pass);
...
//[10-1-2017] Fixed Login Error - Berat ALDIRMAZ
?>
